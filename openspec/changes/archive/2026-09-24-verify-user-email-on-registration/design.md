...
### Keep E2E verification setup data-driven

Ordinary Playwright tests that are not specifically exercising email verification should use fixture or setup data that creates/selects already verified users with `validated_at` set before the test enters the application. They must not rely on a runtime backend bypass endpoint, a special "verify current user" API, or an `APP_E2E_TEST_SUPPORT_ENABLED` flag.

Dedicated verification-flow tests should be the only E2E tests that create unverified accounts through registration and complete verification through Mailpit or another test email capture strategy.

Rationale: the production verification gate should stay meaningful in E2E. A runtime bypass can accidentally become part of the app's operational surface and also makes manually started backends behave differently from the intended test fixture model.
