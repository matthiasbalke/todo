...
- For Playwright e2e runs, first use bash to source `scripts/load-local-domain.sh` from the repo root and resolve `LOCAL_HTTPS_DOMAIN` from the tracked `.local-domain` file.
...
- If the HTTPS deployment is reachable, run Playwright using zsh with `BASE_URL="https://${LOCAL_HTTPS_DOMAIN}" bunx playwright test` from `e2e/`.
