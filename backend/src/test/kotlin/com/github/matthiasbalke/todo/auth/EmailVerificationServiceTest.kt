...
class EmailVerificationServiceTest {
    ...
    @Test
    fun `resending pending email verification replaces token`() {
        val user = User(email = "active@example.com", displayName = "Pending", validatedAt = Instant.now())
        val harness = harness(user)
        harness.service.startPendingEmailChange(user.id, "pending@example.com")
        val firstToken = harness.sent.first { it.recipient == "pending@example.com" }.rawVerificationToken()
        val firstHash = user.pendingEmailToken

        harness.service.requestVerificationEmail(user.id)
        val secondToken = harness.sent.last { it.recipient == "pending@example.com" }.rawVerificationToken()

        assertNotEquals(firstHash, user.pendingEmailToken)
        assertNotEquals(firstToken, secondToken)
        assertFailsVerification("VALIDATION_TOKEN_INVALID") {
            harness.service.submitToken(user.id, firstToken)
        }
    ...
    @Test
    fun `cancelling pending email clears pending fields and preserves active email`() {
        val user = User(
            email = "active@example.com",
            displayName = "Pending",
            validatedAt = Instant.now(),
            pendingEmail = "pending@example.com",
            pendingEmailToken = "hash",
            pendingEmailStarted = Instant.now(),
        )
        val harness = harness(user)

        val state = harness.service.cancelPendingEmail(user.id)

        assertEquals("active@example.com", user.email)
        assertNull(user.pendingEmail)
        assertNull(user.pendingEmailToken)
        assertNull(user.pendingEmailStarted)
        assertNull(state.pendingEmailChange)
    }
