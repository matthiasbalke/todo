...
describe('ListPage menu presentation', () => {
	...
	it('keeps item and category controls for editors but hides list management', async () => {
		...
		const plusIcon = addItem.querySelector('svg');
		expect(plusIcon).not.toBeNull();
		expect(plusIcon).toHaveAttribute('width', '18');
		expect(plusIcon).toHaveAttribute('height', '18');
		expect(addItem).toHaveTextContent('add item');
		expect(addItem).not.toHaveTextContent('+ add item');
