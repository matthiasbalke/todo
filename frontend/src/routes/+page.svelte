...
<div class="flex-1 flex items-center justify-center p-4">
  {#if mode === 'starting'}
    <div class="w-full max-w-sm bg-surface rounded-2xl shadow-sm border border-border-subtle p-8 flex flex-col items-center gap-4">
      <svg class="animate-spin h-8 w-8 text-primary" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path>
      </svg>
      <p class="text-muted text-sm">Application is starting…</p>
    </div>
  {:else}
    <div class="w-full max-w-sm bg-surface rounded-2xl shadow-sm border border-border-subtle p-8">
      <h1 class="text-2xl font-bold text-heading mb-2">Unavailable</h1>
      <p class="text-muted text-sm">Backend did not respond. Please try reloading.</p>
    </div>
  {/if}
</div>
